...
import { Client, Message, Events, MessageFlags, MessageFlagsBitField } from 'discord.js';
...
import { WordleTracker } from './wordleTracker';
		...
		if (interaction.isChatInputCommand()) {
			...
			if (linkedCommand) {
				if (interaction.member || linkedCommand.DM) {
					try {
						...
						if (linkedCommand.followup && !(interactionResponse.flags as MessageFlags[])?.includes(MessageFlags.Ephemeral)) {
			...
			try {
				switch (interactionName) {
					...
					case ('log message'): {
						await interaction.reply(await MessageLogger.parseMessage(interaction.targetMessage))
						break;
					}
					default: {
						await interaction.reply({
